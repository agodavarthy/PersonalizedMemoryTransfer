...
class MovieConversationSimulator(object):
    ...
    def _compute_metrics(self, item_ranks) -> dict:
        ...
        """
        total_precision = {k: [] for k in self._eval_at}
        ...
        if len(item_ranks):
            ...
            for ranks, movies, genre in item_ranks:
                n_relv = len(movies)
                precision = {k: 0 for k in self._eval_at}
                ...
                for i, k in enumerate(self._eval_at):
                    ...
                    total_precision[k].append(precision[k] / k)
                #
        ...
        metrics = {}
        metrics.update({"p@%s" % k: np.mean(v) if len(v) else 0.0 for k, v in total_precision.items()})
...
class BaseConvMovieInterface(object):
    ...
    def observe(self, prev_state: dict, is_seeker: bool, text: str, movie_id: list) -> dict:
...
class ModelRunner(object):
    ...
    def run(self) -> dict:
            ...
            while True:
                ...
                if self.is_training:
                    ...
                    observed = self.model.observe(prev_state,
                                                  ...
                                                  episode['text'] if episode else "", episode['movie_id'] if episode else [])
        ...
        avg_response_time = ((dt.datetime.now()-start)/ timedelta(seconds=1))/_count
        print("\n\nArchana Average Response time = %s" % (avg_response_time))
        print("Archana End Time: %s\n" % dt.datetime.now())
        print(self.simulator.metrics())
