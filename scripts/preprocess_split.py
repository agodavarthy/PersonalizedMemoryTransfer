"""
This is preprocess_conv_data.py modified to perform a train-cross val split
"""
import pickle
...
from util.preprocess import tokenizer, TextEncoder
...
parser.add_argument('-g', '--gpu', help='set gpu device number 0-3', type=str,
                    default="0")
parser.add_argument('-train_filename', '--train_filename', help='output filename for training set', type=str,
                    required=True)
parser.add_argument('-valid_filename', '--valid_filename', help='output filename for validation', type=str,
                    required=True)
...
parser.add_argument('-train_size', '--train_size', help='Max number of examples '
                                                        'in training set (-1 uses all)',
                    type=int, default=-1)
parser.add_argument('-valid_size', '--valid_size', help='validation set size, default 1k',
...
def construct_episodes(movies_list: List[Movie],
    ...
    filename = os.path.join(opt.redial_path, "train_data.jsonl")
    ...
    for idx, mentions in data.movieMentions.iteritems():
        if len(mentions):
            data.at[idx, 'movieMentions'] = {str(k): movieid_to_movie[k].title
            if k in movieid_to_movie else fix_title(v)
                                             for k, v in mentions.items() if v}

    # Preprocess our examples
    print("Starting preprocessing....")
    for _, row in tqdm(data.iterrows(), total=opt.valid_size+opt.train_size):
        # No movies mentioned...
        ...
        if len(example):
            if len(valid_data) < opt.valid_size:
                valid_data.append(example)
            else:
                train_data.append(example)

        if len(train_data) == opt.train_size:
            break

    print(f"\nTrain Episodes: {len(train_data)}")
    print(f"Valid Episodes: {len(valid_data)}\n")
    movies_list = [movie.to_dict() for movie in movies_list]

    print(f"DONE!\nTrain saving to: {opt.train_filename}")
    pickle.dump({'episodes': train_data,
                 'movies': movies_list,
                 }, open(opt.train_filename, 'wb'))
    print(f"Valid saving to: {opt.valid_filename}\n")
    pickle.dump({'episodes': valid_data,
                 'movies': movies_list,
                 }, open(opt.valid_filename, 'wb'))
...
if __name__ == '__main__':
    ...
    if opt.debug:
        ...
        construct_episodes(opt.output, movies_list, encoder, opt)

# inspect = MovieInspector(movies)
# row = inspect.get_row("184464")
# row['title']
# row['plot']
